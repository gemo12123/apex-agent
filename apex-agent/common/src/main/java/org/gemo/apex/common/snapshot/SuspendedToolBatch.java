...
public record SuspendedToolBatch(String sessionId, long turnNo, int iterationNo,
                                 List<PreparedToolCallSnapshot> toolCalls) {
    public SuspendedToolBatch {
        ...
        if (toolCalls.isEmpty()) throw new IllegalArgumentException("toolCalls 不能为空");
        Set<String> ids = new HashSet<>();
        for (PreparedToolCallSnapshot call : toolCalls) {
            if (!ids.add(call.toolCallId())) throw new IllegalArgumentException("toolCallId 不能重复");
        }
        if (toolCalls.stream().noneMatch(call ->
                call.disposition() == PreparedToolCallDisposition.INTERVENTION)) {
