...
class HookContractTest {
    ...
    static Stream<Contract> contracts() {
        return Stream.of(
                ...
                new Contract(HookPoint.PRE_MESSAGE_COMPRESSION, PreMessageCompressionHookResult.class),
                new Contract(HookPoint.POST_MESSAGE_COMPRESSION, PostMessageCompressionHookResult.class),
    ...
    void 运行生命周期结果闭包不得出现Agent定义操作() {
        List<Class<?>> resultRecords = List.of(ContinueLoop.class, EndTurnLoop.class,
                ...
                ContinuePreModelCall.class, EndTurnPreModelCall.class,
                ContinuePostModelCall.class, EndTurnPostModelCall.class,
                ContinuePreToolCall.class, BlockTool.class, ReturnToolResult.class,
                RequestHumanIntervention.class, EndTurnPreToolCall.class,
                ContinuePostToolCall.class, EndTurnPostToolCall.class, ContinueTurnEnd.class);

        for (Class<?> result : resultRecords) {
            for (RecordComponent component : result.getRecordComponents()) {
                assertFalse(AgentDefinitionOperation.class.isAssignableFrom(component.getType()), result.getName());
                assertFalse(component.getGenericType().getTypeName().contains("AgentDefinition"), result.getName());
                assertFalse(component.getGenericType().getTypeName().contains("HookBinding"), result.getName());
            }
        }
    }
