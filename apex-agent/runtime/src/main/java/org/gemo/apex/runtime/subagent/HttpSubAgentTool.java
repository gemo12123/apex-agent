...
public final class HttpSubAgentTool implements AgentTool {
    ...
    public ToolResult execute(ToolCall call, ToolExecutionContext x, ToolExecutionObserver o) {
        ...
        if (trace != null) {
            if (trace.agentKeys().contains(key)) throw new IllegalStateException("SubAgent 调用闭环");
            trace.append(key);
        }
        ...
        var req = HttpRequest.newBuilder(endpoint.resolve("/api/sse/chat")).timeout(timeout).header("Content-Type", "application/json").header("Accept", "text/event-stream").header("X-User-Id", x.userId()).POST(HttpRequest.BodyPublishers.ofString(JsonUtils.toJson(body))).build();
        ...
        try (var reg = x.cancellationToken().onCancel(() -> future.cancel(true))) {
            try {
                var res = future.join();
                if (res.statusCode() / 100 != 2) throw new IllegalStateException("SubAgent HTTP " + res.statusCode());
                StringBuilder content = new StringBuilder();
                var decoder = new SseEventDecoder();
                try (Stream<String> lines = res.body(); var close = x.cancellationToken().onCancel(lines::close)) {
                    lines.forEach(l -> {
                ...
                return new ToolResult(call.toolCallId(), call.name(), content.toString(), Map.of("childSessionId", body.getSessionId()));
    ...
    private void consume(String j, StringBuilder c, ToolExecutionObserver o, CompletableFuture<?> f) {
        ...
        else if (m instanceof InvocationDeclaredMessage || m instanceof InvocationChangeMessage) o.onEvent(m);
    }
}
