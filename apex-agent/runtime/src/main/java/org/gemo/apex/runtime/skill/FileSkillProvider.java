...
public final class FileSkillProvider implements SkillProvider {
    ...
    public FileSkillProvider(Path root) {
        try (Stream<Path> dirs = Files.list(root)) {
            skills = dirs.filter(Files::isDirectory).filter(d -> Files.exists(d.resolve("SKILL.md"))).map(this::load).toList();
        } catch (Exception e) {
    ...
    private SkillDefinition load(Path dir) {
        try {
            ...
            try (Stream<Path> fs = Files.walk(dir)) {
                fs.filter(Files::isRegularFile).filter(p -> !p.getFileName().toString().equals("SKILL.md")).filter(p -> !dir.relativize(p).startsWith("scripts")).forEach(p -> {
                    String n = dir.relativize(p).toString().replace('\\', '/');
                    resources.put(n, new SkillResourceDescriptor(n, "text/plain", p.toUri().toString()));
                });
            }
    ...
    private String value(String md, String key) {
        return md.lines().filter(l -> l.startsWith(key + ":")).map(l -> l.substring(key.length() + 1).trim()).findFirst().orElseThrow();
    }
