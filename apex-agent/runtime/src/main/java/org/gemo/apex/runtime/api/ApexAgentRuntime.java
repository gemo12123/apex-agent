 ...
 *
 * <p>同一 session 的 NEW/HUMAN_RESPONSE 在创建 execution 前同步取得 lease；资源关闭则先阻止 新 execution，再向活动 execution
 * 发出取消请求。
 */
public final class ApexAgentRuntime implements AutoCloseable {
    ...
    ApexAgentRuntime(
            Ports p,
            SessionExecutionCoordinator c,
            AgentEventPublisherFactory f,
            ActiveExecutionRegistry a,
            RuntimeResources r) {
        ports = p;
        coordinator = c;
        publishers = f;
    ...
    private ApexAgentExecution prepare(
            String sid,
            String key,
            String uid,
            RequestType kind,
        ...
        try {
            raw =
                    publishers.create(
                            new AgentExecutionDescriptor(
                                    UUID.randomUUID().toString(), sid, key, uid, kind));
        ...
        try {
            var ex =
                    new ApexAgentExecution(
                            create.apply(ports.create(once, cancel)), cancel, lease, once, active);
            active.register(ex);
            return ex;
