...
public final class InMemoryConversationRepository implements ConversationRepository {
    ...
    public void append(List<AgentMessageEntry> es) {
        ...
        map.compute(es.getFirst().sessionId(), (k, v) -> {
            var s = v == null ? new State() : v.copy();
            for (var e : es) s.add(copy(e));
            return s;
        });
    }
        ...
        void add(AgentMessageEntry e) {
            for (var x : items) {
                if (x.entryId().equals(e.entryId())) {
                    if (!x.equals(e)) throw new IllegalStateException("entryId 冲突");
                    return;
                }
                if (x.sortNo() == e.sortNo()) throw new IllegalStateException("sortNo 冲突");
            }
            items.add(e);
