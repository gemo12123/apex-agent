...
public final class SpringAiModelGateway implements ModelGateway {
    ...
    public ModelResponse stream(ModelRequest r, ModelStreamObserver o) {
        ...
        var callbacks = r.tools().stream().map(t -> (org.springframework.ai.tool.ToolCallback) new org.springframework.ai.tool.ToolCallback() {
            public org.springframework.ai.tool.definition.ToolDefinition getToolDefinition() {
                return new org.springframework.ai.tool.definition.DefaultToolDefinition(t.name(), t.description(), t.inputSchemaJson());
            }

            public String call(String input) {
                throw new IllegalStateException("Spring AI 自动工具执行已关闭");
            }
        }).toList();
        var options = org.springframework.ai.model.tool.ToolCallingChatOptions.builder().toolCallbacks(callbacks).internalToolExecutionEnabled(false).build();
        ...
        Disposable sub = model.stream(new Prompt(ms, options)).subscribe(x -> {
            ...
            for (int i = 0; i < out.getToolCalls().size(); i++)
                calls.add(mapper.fromSpring(out.getToolCalls().get(i), base + i));
            o.onChunk(new ModelStreamChunk(out.getText(), calls.subList(base, calls.size()), Map.of(), false));
        }, x -> {
            failure.set(x);
            done.countDown();
        }, done::countDown);
        try (var reg = o.cancellationToken().onCancel(() -> {
            sub.dispose();
            done.countDown();
        })) {
            try {
        ...
        if (o.cancellationToken().isCancellationRequested()) throw new CancellationRequestedException();
        if (failure.get() != null) throw new IllegalStateException("模型调用失败", failure.get());
