...
class ToolConfirmationContractTest {
    ...
    void buildsCompleteConfirmationDisplayContractUsingCorePreallocatedId() {
        ToolCall call =
                KitFixtures.call("meeting_tool", Map.of("room", "A1001", "date", "2026-08-02"));
        HookBinding binding =
                KitFixtures.binding(
                        ToolConfirmHook.REGISTRATION_NAME,
                        List.of("meeting_tool"),
                        Map.of(
                                "title",
                                "确认会议",
                                "description",
                                ...
                                "拒绝",
                                "display-fields",
                                List.of(Map.of("key", "date", "label", "日期")),
                                "editable-fields",
                                List.of(
                                        Map.of(
                                                "key",
                                                ...
                                                true,
                                                "options",
                                                List.of(Map.of("label", "A1001"))))));

        RequestHumanIntervention result =
                assertInstanceOf(
                        RequestHumanIntervention.class,
                        hook.apply(KitFixtures.pre(call, binding, null)));
        ToolConfirmationInterventionRequest request =
                assertInstanceOf(ToolConfirmationInterventionRequest.class, result.request());
        ...
        assertEquals(
                EditableFieldInputType.SINGLE_SELECT,
    ...
    void usesDeterministicReadOnlyDefaultDisplayValues() {
        ...
        ToolConfirmationInterventionRequest request =
                assertInstanceOf(
                        ToolConfirmationInterventionRequest.class,
                        assertInstanceOf(
                                        RequestHumanIntervention.class,
                                        hook.apply(
                                                KitFixtures.pre(
    ...
    void continuesWithoutGeneratingStateMachineResultOnResumeReentrancy() {
        ToolCall call = KitFixtures.call("search", Map.of("query", "apex"));
        ToolConfirmationSubmission submission =
                new ToolConfirmationSubmission(
                        call.toolCallId(),
                        "intervention-1",
                        ConfirmationDecision.CONFIRM,
                        Map.of("query", "updated"));
        assertInstanceOf(
