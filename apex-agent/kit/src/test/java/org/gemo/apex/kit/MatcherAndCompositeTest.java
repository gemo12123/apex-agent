...
class MatcherAndCompositeTest {
    ...
    void matchesExactNamesAndWildcardAndRejectsInvalidGlob() {
        ...
        assertThrows(IllegalArgumentException.class, () -> new GlobToolMatcher(List.of("search_*")));
    ...
    void executesCompositeInExplicitOrderAndStopsAtTerminalResult() {
        ...
        LifecycleHook<PreToolCallContext, PreToolCallHookResult> never = hook(calls, "never", continueResult());
    ...
    void propagatesCompositeExceptionsAndRejectsDifferentDescriptors() {
        ...
        LifecycleHook<PreToolCallContext, PreToolCallHookResult> mismatched = new LifecycleHook<>() {
            @Override public HookTypeDescriptor descriptor() {
                return new HookTypeDescriptor(HookPoint.POST_TOOL_CALL,
                        PreToolCallContext.class, PreToolCallHookResult.class);
            }
            @Override public PreToolCallHookResult apply(PreToolCallContext context) { return continueResult(); }
        };
    ...
    private PreToolCallContext context() {
        return KitFixtures.pre(KitFixtures.call("search", Map.of()),
                KitFixtures.binding("hook", List.of("search"), Map.of()), null);
    }
}
