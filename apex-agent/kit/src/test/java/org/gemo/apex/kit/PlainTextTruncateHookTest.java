...
class PlainTextTruncateHookTest {
    ...
    void preservesTextWithinLimitAndMarksTruncationWithinLimit() {
        ...
        assertEquals(
                "12345",
                apply(hook, new ToolResult("call-1", "tool", "12345", Map.of())).patch().content());
        String truncated =
                apply(hook, new ToolResult("call-1", "tool", "123456", Map.of())).patch().content();
    ...
    void truncatesByUnicodeCodePointsWithoutSplittingEmoji() {
        String truncated =
                apply(
                                new PlainTextTruncateHook(3),
                                new ToolResult("call-1", "tool", "A😀BC", Map.of()))
                        .patch()
                        .content();
    ...
    void keepsNonTextResultsUnchangedWhenOverLimit() {
        ToolResult result =
                new ToolResult(
                        ...
                        "123456",
                        Map.of(
                                PlainTextTruncateHook.MESSAGE_TYPE_METADATA_KEY,
