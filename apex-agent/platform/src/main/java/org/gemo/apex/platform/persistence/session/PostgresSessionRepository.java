...
public class PostgresSessionRepository implements SessionRepository {
    ...
    public PostgresSessionRepository(JdbcTemplate jdbc) { this.jdbc = jdbc; }

    @Override
    public Optional<SessionSnapshot> load(String sessionId) {
        var rows = jdbc.query("SELECT * FROM apex_agent_session WHERE session_id = ?", (rs, row) ->
                new AgentSessionEntity(rs.getString("session_id"), rs.getString("user_id"),
                        rs.getString("agent_key"), rs.getString("status"), rs.getLong("current_turn_no"),
                        rs.getString("agent_definition_snapshot"), rs.getString("enabled_tool_names"),
                        rs.getString("activated_skill_names"), rs.getString("runtime_snapshot"),
                        rs.getString("suspended_tool_call"), rs.getTimestamp("last_active_time").toInstant()), sessionId);
    ...
    public void save(SessionSnapshot snapshot) {
        ...
        jdbc.update("""
                ...
                """, entity.sessionId(), entity.userId(), entity.agentKey(), entity.status(), entity.currentTurnNo(),
                entity.agentDefinitionSnapshot(), entity.enabledToolNames(), entity.activatedSkillNames(),
                entity.runtimeSnapshot(), entity.suspendedToolCall(), Timestamp.from(entity.lastActiveTime()),
