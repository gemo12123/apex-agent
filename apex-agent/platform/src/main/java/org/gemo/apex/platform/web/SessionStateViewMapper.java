...
public final class SessionStateViewMapper {
    public SessionStateView map(SessionSnapshot snapshot) {
        HumanInterventionMessage pending = snapshot.suspendedToolBatch() == null
                ? null : interaction(snapshot);
    ...
    private HumanInterventionMessage interaction(SessionSnapshot snapshot) {
        ...
        for (var prepared : snapshot.suspendedToolBatch().toolCalls()) {
            ...
            if (prepared.intervention() instanceof QuestionInterventionRequest request) {
                List<AskHumanQuestionDetail> questions = request.questions().stream().map(spec -> {
                    List<AskHumanOption> options = spec.options().stream().<AskHumanOption>map(option ->
                            AskHumanOption.builder().label(String.valueOf(option.get("label")))
                                    .description(option.get("description") == null ? null
                                            : String.valueOf(option.get("description"))).build()).toList();
                    return AskHumanQuestionDetail.builder().inputType(spec.inputType())
                            .question(spec.question()).description(spec.description()).options(options).build();
                }).toList();
                messages.add(AskHumanInterventionDetail.builder().toolCallId(request.toolCallId())
        ...
        if (messages.isEmpty()) throw new IllegalStateException("人工介入批次不包含待处理项");
        return HumanInterventionMessage.builder().context(Map.of("mode", "react"))
