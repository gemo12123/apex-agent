...
public class AgentController {
    ...
    public AgentController(AgentDefinitionProvider definitions) { this.definitions = definitions; }

    @GetMapping(value = "/agents", produces = MediaType.APPLICATION_JSON_VALUE)
    public ApiResponse<?> agents() {
        return ApiResponse.success(definitions.listAgents().stream()
                .map(value -> Map.of("agentKey", value.agentKey(), "name", value.name())).toList());
    }
}
