...
public final class SpringPropertiesAgentDefinitionProvider implements AgentDefinitionProvider {
    ...
    public SpringPropertiesAgentDefinitionProvider(ApexAgentPlatformProperties properties,
                                                   ResourceLoader resourceLoader) {
        if (properties.getDefinitionResource() != null && !properties.getDefinitionResource().isBlank()) {
        ...
        properties.getAgents().forEach((key, value) -> result.put(key, convert(key, value, resourceLoader)));
    ...
    public AgentDefinition load(String agentKey) {
        ...
        if (definition == null) throw new IllegalArgumentException("Agent 不存在: " + agentKey);
        return definition;
    }
    ...
    private static AgentDefinition convert(String key, ApexAgentPlatformProperties.Agent source,
                                           ResourceLoader loader) {
        if (source == null) throw new IllegalArgumentException("Agent 配置不能为空: " + key);
        String systemPrompt = read(source.getPrompt() == null ? null : source.getPrompt().getSystem(), loader);
        ...
        source.getSubAgents().forEach((name, value) -> subAgents.put(name,
                new SubAgentDefinition(value.getAgentKey(), value.getDescription())));
        Map<HookPoint, List<HookBinding>> hooks = new LinkedHashMap<>();
        source.getHooks().forEach((point, bindings) -> hooks.put(HookPoint.valueOf(point), bindings.stream()
                .map(binding -> new HookBinding(binding.getId(), binding.getName(), binding.getOrder(),
                        binding.isEnabled(), binding.getTools(), binding.getOptions())).toList()));
        return new AgentDefinition(DefinitionSchemaVersion.V1,
                ...
                new MessageCompressionDefinition(compression.isEnabled(), compression.getMaxMessages()),
                new ToolSetDefinition(tools.getAvailable(), tools.getDefaultEnabled()),
                source.getSkills(), subAgents, hooks);
    }

    private static String read(String location, ResourceLoader loader) {
        if (location == null || location.isBlank()) throw new IllegalArgumentException("prompt.system 不能为空");
