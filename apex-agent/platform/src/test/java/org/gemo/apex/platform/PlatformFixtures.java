...
public final class PlatformFixtures {
    ...
    public static AgentMessageEntry userMessage(String content) {
        return new AgentMessageEntry("entry-1", "session-1", 1, 0, MessageRole.USER,
                MessageType.TEXT, content, Map.of("nested", List.of("value")), NOW);
    }
    ...
    public static SessionSnapshot suspendedSnapshot() {
        AgentDefinitionRecoverySnapshot definition = new AgentDefinitionRecoverySnapshot(SnapshotSchemaVersion.V1,
                new AgentMetadata("default", "Default", "Default agent"),
                new PromptDefinition("system", 4), new MessageCompressionDefinition(true, 20),
                Set.of("search"), Set.of(), Map.of(), Map.of());
        ModelRequest request = new ModelRequest("system", List.of(userMessage("hello")), List.of(), Map.of());
        ...
        IterationSnapshot iteration = new IterationSnapshot(1, IterationStatus.SUSPENDED,
                request, response, List.of(), NOW, null);
    ...
    public static SessionSnapshot confirmationSnapshot() {
        ...
        ToolConfirmationDetail detail = ToolConfirmationDetail.builder().confirmationId("confirmation-1")
                .toolCallId("call-1").invocationId("invocation-1").toolName("search")
                .toolDisplayName("搜索").title("确认搜索").riskLevel("MEDIUM").editable(false)
