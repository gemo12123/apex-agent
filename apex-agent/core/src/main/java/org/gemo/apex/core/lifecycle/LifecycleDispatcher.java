...
public final class LifecycleDispatcher {
    ...
    public LifecycleDispatchOutcome dispatch(HookPoint point, ApexAgentContext context,
        ...
        for (HookBinding binding : bindings) {
            ...
            if (hook == null || hook.descriptor() == null || hook.descriptor().hookPoint() != point) {
            ...
            try {
                ...
                LOG.log(System.Logger.Level.WARNING,
                        "生命周期 Hook 执行失败，已跳过: point=" + point + ", hookId=" + binding.id(), error);
                continue;
            }
    ...
    private LifecycleDispatchOutcome validateAndApply(HookPoint point, ApexAgentContext context, Object result) {
        ...
        return switch (result) {
            case ContinueLoop value -> { applyMutations(context, value.mutations()); yield continued(); }
            case EndTurnLoop value -> end(value.reason());
            case ContinuePreModelCall value -> {
                applyMutations(context, value.mutations()); context.modelRequest(value.patch().replacement());
                yield continued();
            }
            ...
            case ContinuePostModelCall value -> {
                ...
                applyMutations(context, value.mutations()); context.modelResponse(value.patch().replacement());
                yield continued();
            }
            ...
            case ContinuePreToolCall value -> {
                ...
                context.toolCall(new ToolCall(call.toolCallId(), call.name(), call.ordinal(),
                        value.patch().arguments(), call.metadata()));
                yield continued();
            }
            ...
            case RequestHumanIntervention value -> new LifecycleDispatchOutcome.HumanIntervention(value.request());
            case EndTurnPreToolCall value -> end(value.reason());
            case ContinuePostToolCall value -> {
                applyMutations(context, value.mutations());
                ToolResult old = context.toolResult();
                context.toolResult(new ToolResult(old.toolCallId(), old.toolName(),
                        value.patch().content(), value.patch().metadata()));
                yield continued();
            }
            ...
            default -> throw new HookContractException("生命周期结果族不受支持: " + result.getClass().getName());
        };
    }
    ...
    private void applyMutations(ApexAgentContext context, HookMutations mutations) {
        ...
        List<AgentMessageEntry> messages = current == null ? new ArrayList<>() : new ArrayList<>(current.messages());
        ...
        context.enableTools(mutations.toolActivationDelta().enable(), mutations.toolActivationDelta().disable());
        if (current != null) {
            context.modelRequest(new ModelRequest(current.systemPrompt(), messages, current.tools(), current.options()));
        }
    }
    ...
    private void validateResponseAssociation(ModelResponse before, ModelResponse after) {
        ...
        for (int i = 0; i < before.toolCalls().size(); i++) {
            ...
            if (!left.toolCallId().equals(right.toolCallId()) || !left.name().equals(right.name())) {
    ...
    private void validateToolResult(ToolCall call, ToolResult result) {
        if (!call.toolCallId().equals(result.toolCallId()) || !call.name().equals(result.toolName())) {
            throw new HookContractException("ToolResult 与 ToolCall 不关联");
        }
    }

    private LifecycleDispatchOutcome continued() { return new LifecycleDispatchOutcome.Continued(); }
    private LifecycleDispatchOutcome end(String reason) { return new LifecycleDispatchOutcome.EndTurn(reason); }
}
