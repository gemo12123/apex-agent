...
public final class AgentDefinitionAssembler {
    private static final System.Logger LOG = System.getLogger(AgentDefinitionAssembler.class.getName());
    private final AgentDefinitionValidator validator = new AgentDefinitionValidator();

    public AgentAssemblyResult assemble(String sessionId, String agentKey,
                                        Optional<SessionSnapshot> existingSession, AgentPorts ports) {
        AgentDefinition source = ports.definitionProvider().load(agentKey);
        if (source == null) throw new InvalidAgentDefinitionException("找不到 Agent 定义: " + agentKey);
        ...
        Classification classification = classifyUnavailable(candidate, loadedCatalog,
                ports.toolAvailabilityProvider().current(), existingSession, ports);
        validator.validate(classification.definition(), classification.catalog(), ports);
        Set<String> enabled = existingSession
        ...
        if (existingSession.isPresent()) {
            ...
            Set<String> historicalNames = classification.history().stream()
                    .map(HistoricalToolBinding::toolName).collect(java.util.stream.Collectors.toSet());
            retained.removeIf(name -> !classification.definition().tools().availableTools().contains(name)
        ...
        return new AgentAssemblyResult(new AgentDefinitionSnapshot(classification.definition()), enabled,
                classification.history(), classification.catalog());
    }

    @SuppressWarnings({"rawtypes", "unchecked"})
    private void dispatchAgentBuild(String sessionId, AgentDefinition source,
                                    AgentDefinitionDraft draft, AgentPorts ports) {
        List<HookBinding> snapshot = source.hooks().getOrDefault(HookPoint.AGENT_BUILD, List.of()).stream()
        ...
        for (HookBinding binding : snapshot) {
            ...
            try {
                ...
                LOG.log(System.Logger.Level.WARNING, "AGENT_BUILD Hook 执行失败，已跳过: " + binding.id(), error);
            }
        }
    }
    ...
    private AgentDefinition materialize(AgentDefinition source, AgentDefinitionDraft draft) {
        ...
        return new AgentDefinition(source.schemaVersion(), source.metadata(), draft.prompt(),
                source.messageCompression(), new ToolSetDefinition(draft.availableTools(), defaults),
                source.enabledSkills(), source.subAgents(), draft.hooks());
    }

    private Classification classifyUnavailable(AgentDefinition candidate, ToolCatalog catalog,
        ...
        List<HistoricalToolBinding> history = new ArrayList<>(existingSession
                .map(SessionSnapshot::historicalToolBindings).orElse(List.of()));
        for (String name : candidate.tools().availableTools()) {
            ...
            UnavailableToolSource source = availability.unavailableSources().stream()
                    .filter(item -> name.startsWith(item.stableNamePrefix())).findFirst().orElse(null);
            boolean unavailable = availability.unavailableToolNames().contains(name) || source != null;
            boolean existed = existingSession.map(session -> session.activeDefinition().availableTools().contains(name))
                    .orElse(false);
            if (!unavailable) throw new InvalidAgentDefinitionException("工具无法解析且没有不可用来源: " + name);
            if (!existed) throw new UnavailableToolBindingException(name);
            effective.remove(name);
            HistoricalToolBinding binding = new HistoricalToolBinding(name,
                    source == null ? ToolOrigin.LOCAL : source.origin(),
                    source == null ? name : source.sourceId(),
                    source == null ? "UNAVAILABLE" : source.reasonCode(), ports.timeProvider().now());
            if (history.stream().noneMatch(item -> item.identity().equals(binding.identity()))) history.add(binding);
        }
        Set<String> defaults = new LinkedHashSet<>(candidate.tools().defaultEnabledTools());
        defaults.retainAll(effective);
        AgentDefinition result = new AgentDefinition(candidate.schemaVersion(), candidate.metadata(),
                candidate.prompt(), candidate.messageCompression(), new ToolSetDefinition(effective, defaults),
                candidate.enabledSkills(), candidate.subAgents(), candidate.hooks());
        List<org.gemo.apex.extension.tool.AgentTool> effectiveTools = catalog.ordered().stream()
                .filter(tool -> effective.contains(tool.definition().name())).toList();
        return new Classification(result, new ToolCatalog(effectiveTools), history);
    }

    private record Classification(AgentDefinition definition, ToolCatalog catalog,
