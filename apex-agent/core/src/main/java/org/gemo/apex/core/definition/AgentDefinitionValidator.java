...
public final class AgentDefinitionValidator {
    ...
    private static final Map<HookPoint, Class<? extends HookContextView>> CONTEXT_TYPES = Map.ofEntries(
            ...
            Map.entry(HookPoint.PRE_MESSAGE_COMPRESSION, PreMessageCompressionContext.class),
            Map.entry(HookPoint.POST_MESSAGE_COMPRESSION, PostMessageCompressionContext.class),
    ...
    private static final Map<HookPoint, Class<? extends LifecycleHookResult>> RESULT_TYPES = Map.ofEntries(
            ...
            Map.entry(HookPoint.PRE_MESSAGE_COMPRESSION, PreMessageCompressionHookResult.class),
            Map.entry(HookPoint.POST_MESSAGE_COMPRESSION, PostMessageCompressionHookResult.class),
    ...
    public void validate(AgentDefinition definition, ToolCatalog catalog, AgentPorts ports) {
        ...
        Set<String> skillNames = ports.skillProvider().loadSkills().stream()
    ...
    private void validateBindings(AgentDefinition definition, AgentPorts ports, Set<HookPoint> points) {
        for (HookPoint point : points) {
            ...
            for (HookBinding binding : definition.hooks().getOrDefault(point, java.util.List.of())) {
                ...
                if (hook == null || hook.descriptor() == null
                    ...
                    throw new InvalidAgentDefinitionException("Hook descriptor 不匹配: " + binding.id());
                }
            }
        }
    }
