...
class HumanInterventionExecutionTest {
    ...
    void runsPostAndClearsSuspensionWhenResumingBlockOrReturn() {
        ...
        Scenario returned = questionScenario(context -> new ReturnToolResult(
                new ToolResult(context.toolCall().toolCallId(), context.toolCall().name(), "直接结果", Map.of())));
    ...
    void readsTypedAnswersFromAskHumanAndContinuesNextIterationWhenAllResumeResultsContinue() {
        ...
        scenario.fixture.tool("ask", (call, context, observer) -> {
    ...
    void mergesOnlyEditableParametersAndRejectsFixedResultsWhenToolConfirmationApproved() {
        ...
        ApexAgent approvedAgent = new ApexAgentFactory().createResumed(command(Map.of(
                "interaction_type", "TOOL_CONFIRMATION", "confirmation_id", confirmationId(approved),
                "decision", "APPROVE", "updated_args", Map.of("room", "B", "locked", "changed"))),
        ...
        Scenario denied = confirmationScenario(context -> continued(context), new AtomicReference<>());
        assertInstanceOf(AgentRunOutcome.Suspended.class, denied.fresh.run());
        ApexAgent deniedAgent = new ApexAgentFactory().createResumed(command(Map.of(
                "interaction_type", "TOOL_CONFIRMATION", "confirmation_id", confirmationId(denied),
                "decision", "DENY")), denied.fixture.ports());
    ...
    void preservesPriorResultsAndContinuesSubsequentCallsWhenResumingMultipleToolCalls() {
        ...
        fixture.tool("ask", (call, context, observer) ->
                new ToolResult(call.toolCallId(), call.name(), call.toolCallId(), Map.of()));
        fixture.hooks.put("first", preHook(context ->
                new RequestHumanIntervention(question(context.toolCall()))));
        fixture.definition = fixture.definition(Map.of(HookPoint.PRE_TOOL_CALL,
                        List.of(new HookBinding("pre-1", "first", 0, true, List.of("ask"), Map.of()))),
                Set.of("plain", "ask"), Set.of("plain", "ask"));
        fixture.modelResponses.add(new ModelResponse("", List.of(
                ...
                new ToolCall("call-2", "plain", 2, Map.of(), Map.of())), Map.of()));
    ...
    void migratesSuspendedToolToHistoryWithoutExecutingItWhenItBecomesUnavailableBeforeResume() {
        ...
        assertEquals(List.of("ask"), resumed.snapshot().historicalToolBindings().stream()
    ...
    private Scenario questionScenario(Function<PreToolCallContext, PreToolCallHookResult> second,
        ...
        return scenario(fixture, context -> new RequestHumanIntervention(question(context.toolCall())),
                second, twoCalls);
    }

    private Scenario confirmationScenario(Function<PreToolCallContext, PreToolCallHookResult> second,
        ...
        return scenario(fixture, context -> new RequestHumanIntervention(confirmation(context)), second, false);
    }

    private Scenario scenario(CoreTestFixture fixture,
        ...
        fixture.definition = fixture.definition(Map.of(
                HookPoint.PRE_TOOL_CALL, List.of(binding("pre-1", "first", 0), binding("pre-2", "second", 1)),
                HookPoint.POST_TOOL_CALL, List.of(binding("post-1", "post", 0))),
                Set.of("ask"), Set.of("ask"));
        List<ToolCall> calls = new ArrayList<>();
        calls.add(new ToolCall("call-1", "ask", 0,
                Map.of("value", "patched", "room", "A", "locked", "original"), Map.of()));
        if (twoCalls) calls.add(new ToolCall("call-2", "ask", 1, Map.of(), Map.of()));
    ...
    private LifecycleHook<PreToolCallContext, PreToolCallHookResult> preHook(
        ...
        return new LifecycleHook<>() {
            ...
            @Override public PreToolCallHookResult apply(PreToolCallContext context) { return action.apply(context); }
        };
    }
    ...
    private PreToolCallHookResult continued(PreToolCallContext context) {
        return new ContinuePreToolCall(HookMutations.none(), new ToolCallPatch(context.toolCall().arguments()));
    }

    private QuestionInterventionRequest question(ToolCall call) {
        return new QuestionInterventionRequest(call.toolCallId(),
                List.of(new QuestionSpec("TEXT_INPUT", "请选择", null, List.of())));
    }
    ...
    private ToolConfirmationInterventionRequest confirmation(PreToolCallContext context) {
        ToolConfirmationDetail detail = ToolConfirmationDetail.builder()
                .confirmationId(context.proposedInterventionId()).toolCallId(context.toolCall().toolCallId())
                .invocationId(context.invocationId()).toolName(context.toolCall().name())
                .toolDisplayName("ask").title("确认").description("确认执行")
                .riskLevel("MEDIUM").editable(true).confirmLabel("批准").denyLabel("拒绝")
                .displayFields(List.of()).editableFields(List.of()).build();
        return new ToolConfirmationInterventionRequest(context.toolCall().toolCallId(),
                context.proposedInterventionId(), context.invocationId(), context.toolCall().name(), detail,
                Set.of("room"));
    }

    private ApexAgent resumeQuestion(Scenario scenario) {
        scenario.fixture.modelResponses.add(new ModelResponse("完成", List.of(), Map.of()));
        return new ApexAgentFactory().createResumed(command(Map.of(
                "interaction_type", "ASK_HUMAN", "answers", Map.of("0", "A"))), scenario.fixture.ports());
    }
    ...
    private List<String> toolContents(CoreTestFixture fixture) {
        return fixture.conversation.stream().filter(entry -> entry.messageType() == MessageType.TOOL_RESULT)
