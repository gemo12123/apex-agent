...
运行时层级为 `Session → Turn → Iteration`。一个 Iteration 表示一个 Turn 内的一次模型推理及其工具调用处理；模型返回工具调用并继续执行循环时，下次模型推理会创建新的 Iteration。人在回路挂起和恢复会继续使用原 Iteration。
...
- 处理 Hook 流控动作：`BLOCK_TOOL`、`SKIP_ITERATION`、`END_TURN`、`REQUEST_CONFIRMATION`
...
生命周期 Hook 的分发器是 `hook/lifecycle/DefaultAgentLifecycleHookRuntime`，覆盖 `HookPoint` 的 8 个生命周期点（turn/iteration/model/tool 前后）。对工具而言：
