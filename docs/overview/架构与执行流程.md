...
- ModelRequest 从当前定义、AGENT_BUILD 前置消息、会话有效窗口生成；发送顺序固定为根 system prompt、`prefixDeveloperMessages`、摘要及对话消息，摘要不会拼入 system prompt。每个 NEW/恢复 execution 只从数据库装配一次窗口，同一 execution 后续 Iteration 直接读取 Context。
...
- 普通消息和压缩结果都先预计算下一 Context 窗口，再写 Conversation；写入成功后替换 Context，随后保存 Session。压缩后的窗口立即变为累计摘要与保留尾部，后续模型调用不再重新查库。
