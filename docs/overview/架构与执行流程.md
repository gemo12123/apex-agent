...
- 普通消息和压缩结果都先预计算下一 Context 窗口，再通过原子 `ConversationWriteBatch` 写 Conversation；写入成功后替换 Context，随后保存 Session。POST 压缩的摘要与消息操作共同提交。压缩后的窗口立即变为累计摘要与保留尾部，后续模型调用不再重新查库，压缩边界不会拆分工具调用与结果组。
