...
- AGENT_BUILD 使用原始 Binding 的不可变排序快照；本次分发中的定义修改只影响最终定义和后续生命周期。它可通过 `AppendPrefixDeveloperMessage` 顺序追加 SYSTEM/USER 临时文本，每个 NEW 或恢复请求重新构建，同一请求内各 Iteration 复用。
- 其他生命周期不能增删定义 Binding。
- 普通 Hook 异常记录 warn 并跳过当前 Binding；违反流程契约的错误终止执行。

## 模型、压缩与工具

- ModelRequest 从当前定义、AGENT_BUILD 前置消息、会话有效窗口生成；发送顺序固定为根 system prompt、`prefixDeveloperMessages`、摘要及对话消息，摘要不会拼入 system prompt。每个 NEW/恢复 execution 只从数据库装配一次窗口，同一 execution 后续 Iteration 直接读取 Context。调用 ModelGateway 前强制校验 `ModelRequest.messages == ConversationWindow.messages`，工具定义与当前 enabledTools 一致。
- 默认消息压缩通过当前 ModelGateway 的独立无工具请求生成累计摘要，只输入已有摘要和本次被淘汰消息，不递归进入 ReAct 循环。
- 仓储加载摘要及 `compacted=FALSE` 的未压缩消息；窗口验证这些消息位于摘要范围之后，再插入一条独立 `SUMMARY` 消息。
- Agent 级压缩由消息数、可选 token 阈值、可选字符阈值按 OR 关系触发；容量阈值只触发压缩，不拒绝模型调用。
- `prefixDeveloperMessages` 计入 token/字符容量，但不计入消息数阈值、不进入摘要或淘汰集合，并在写入任何 Session/Iteration 快照前剥离。
- `prefixDeveloperMessages` 是唯一请求级临时消息例外；Hook 的普通 Append/Replace/Remove 全部通过 ConversationRepository 持久化，后续 Turn 可见。
- 普通消息和压缩结果都先预计算下一 Context 窗口，再通过原子 `ConversationWriteBatch` 写 Conversation；写入成功后替换 Context，随后保存 Session。POST 压缩的摘要与消息操作共同提交。压缩后的窗口立即变为累计摘要与保留尾部，后续模型调用不再重新查库。Hook 可以编辑当前工具消息组，Core 不锁定或校验编辑后的 ToolCall/ToolResult 关系，协议完整性由 Hook 负责。
...
- `apex_agent_session`：所有权、状态、定义快照、工具/Skill 集合和 runtime 快照。挂起工具批次写入 `runtime_snapshot.suspendedToolBatch`；既有 `suspended_tool_call` TEXT 列仅用于读取历史批次，新写入为 `NULL`，不新增迁移。
...
默认生产事件为 STREAM_CONTENT、HUMAN_INTERVENTION、INVOCATION_DECLARED/CHANGE（工具/子调用路径）、循环异常时的 TASK_ERROR 和 END。兼容 DTO 仍识别 STREAM_THINK 与 ARTIFACT，但默认运行不生产；PLAN 与 TASK_THINK 事件已从协议删除。

END 不携带 `execution_status`。前端保留 TASK_ERROR 设置的 error 状态，否则按有序 `pendingInterventions` 推断统一的 `waiting-intervention`，并使用 `apex:active-session:v1` + 只读状态接口恢复刷新后的整批 HITL 卡片。
