...
- ModelRequest 从未压缩窗口、摘要和当前定义生成，并受硬上限保护。
- 压缩先写 Conversation，再写 Session，然后才调用模型；任一步失败都停止传播。
- Spring AI Adapter 关闭自动工具执行，core 是唯一工具编排者。
- 多个 ToolCall 按 ordinal 顺序执行；Hook 可以改参数、请求确认或截断结果。
- 工具中途不可用时生成标准 ToolResult，保持一一配对。
- 达到最大轮次且仍有未完成工具时，由 core 唯一工厂补固定强制结束结果。

## 挂起与恢复

挂起顺序固定为：保存完整 SessionSnapshot -> 发布 ASK_HUMAN/TOOL_CONFIRMATION -> END 当前传输。

恢复流程：

1. `resumeAgent` 与 NEW 竞争同一 lease。
2. 校验 userId、agentKey、sessionId、`HUMAN_IN_THE_LOOP` 状态和快照版本。
3. 解析 typed submission，恢复同一 Turn/Iteration/ToolCall 与已执行 Hook 进度。
4. ASK_HUMAN、确认批准、确认拒绝、再次挂起、全部继续五类分支均回到唯一 ReAct 链。
...
- Spring AI dispose reactive subscription 并唤醒等待线程；MCP 取消 call handle；HTTP SubAgent 取消 future/stream。
...
- `apex_agent_session`：所有权、状态、定义快照、工具/Skill 集合、runtime 快照、挂起工具。
...
默认生产事件为 STREAM_CONTENT、ASK_HUMAN、TOOL_CONFIRMATION、INVOCATION_DECLARED/CHANGE（工具/子调用路径）和 END。兼容 DTO 仍识别 STREAM_THINK、PLAN、TASK_THINK、ARTIFACT，但默认运行不生产。

END 不携带 `execution_status`。前端按 pending prompts/confirmations 推断等待态，并使用 `apex:active-session:v1` + 只读状态接口恢复刷新后的两类 HITL 卡片。

## 运行边界

- platform 只能单实例，`replicas=1`，升级停旧再启新。
- 远程 SubAgent 不支持嵌套 HITL。
