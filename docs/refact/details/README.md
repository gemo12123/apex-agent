...
2026-07-31 与 2026-08-01 已确认并写入全部分册：
...
- Spring AI 先按实际依赖/API 基线收敛；只有 convergence、编译或真实 Adapter 契约证明不兼容时才允许最小调整 Spring AI BOM/版本属性，不扩大到 Spring Boot 或模型供应商 SDK。

D-01～D-09 均已关闭；实施期仍需按 [冲突、风险与待确认项](11-冲突风险与待确认项.md) 中的风险控制和证据门槛执行。
