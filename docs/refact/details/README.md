...
本详细设计对若干上游未闭合问题给出推荐答案，并按推荐答案展开类型和流程。实施前应集中确认 [冲突、风险与待确认项](11-冲突风险与待确认项.md) 中标为 `阻塞` 的条目，尤其是：

- 已失效 MCP/SubAgent 工具与不可变定义、旧 session 状态之间的清理机制。
- core 构造失败时 platform 是返回仅含 END 的 SSE，还是改为 HTTP 错误。
- 生命周期“可修改 Hook”是只允许 AGENT_BUILD 修改定义，还是允许运行期重写 Hook 链。

未确认前可以完成 FND、PRO、COM、EXT 的不冲突部分，但不得实现会固化上述语义的 core/runtime/platform 分支。
